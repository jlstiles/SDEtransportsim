...
#' @export
SDE_glm = function(data, forms, RCT = 0.5,transport, pooled, gstar_S = 1, truth, B = NULL, max_iter = 100) 
{
    ...
    gstar_info = get_gstarM_glm_eff(data = data, forms = forms, 
                                  transport = transport, pooled = pooled, gstar_S = gstar_S, truth = NULL)
    ...
    initdata = get.mediation.initdata_glm(data = data, forms = forms, RCT = RCT, transport = transport, 
                                             pooled = pooled, gstarM_astar)

    est_info = mediation.steps_glm(initdata, transport, bootstrap = FALSE, max_iter)
    ...
    if (!is.null(B)) {
      boot_ests = lapply(1:B, FUN = function(x) {
        ...
        est_info = mediation.steps_glm(initdata, transport, bootstrap = TRUE, max_iter)
      ...
      boot_ests = do.call(rbind, boot_ests[1:4])
