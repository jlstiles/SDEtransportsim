# Simulation function
#' @export
SDE_glm_eff = function(data, truth = NULL, truncate = list(lower =.0001, upper = .9999), 
                    B = 500, forms, RCT = 0.5) 
{
  L = truncate$lower
  U = truncate$upper
  ...
  gstar_info = get_gstarM_glm_eff(data = data, truth = truth, forms = forms)
  ...
  init_info = get.mediation.initdata_glm_eff(data = data, forms = forms, RCT = RCT)
  ...
  est_info = lapply(0:1, FUN = function(astar) {
    lapply(0:1, FUN = function(a) {
      ...
      EE_gcomp_info = mediation.step2_glm_eff(data = data, Qstar_M = Y_preds[[1]], 
      ...
      tmle_info = mediation.step2_glm_eff(data = data, Qstar_M = update$Qstar_M, 
  ...
  if (!is.null(B)) {
    ...
    est_info = lapply(0:1, FUN = function(astar) {
      return(lapply(0:1, FUN = function(a) {
        update = mediation.step1_glm_eff(initdata = init_info$initdata, Y_preds = Y_preds, data = data, 
                                 gstarM_astar[[astar+1]], a)
        iptw_info = update$est_iptw
        
        Y_Mg = get.stochasticM(gstarM_astar[[astar+1]], Y_preds[[2]], Y_preds[[3]]) 
        A_ps = init_info$initdata$A_ps
        EE_mle_info = mediation.step2_glm_eff(data = data, Qstar_M = Y_preds[[1]], 
                                      Qstar_Mg = Y_Mg, Hm = update$Hm, A_ps = A_ps, 
                                      a = a, tmle = FALSE,
...
#' @export
get_gstarM_glm_eff  = function(data, truth, forms) 
{
...
#' @export
get.mediation.initdata_glm_eff = function(data, forms, RCT = 0.5) {
...
#' @export
mediation.step1_glm_eff = function(initdata, Y_preds, data, gstarM_astar, a) {
...
#' @export
mediation.step2_glm_eff = function(data, Qstar_M, Qstar_Mg, Hm, A_ps, a, tmle = TRUE,
