...
SDE_tmle1 = function(data, a, a_star, sl, V=10, covariates, truth = NULL, 
                    truncate = list(lower =.0001, upper = .9999), glm_only = TRUE,
                    iptw = FALSE, onestep = FALSE) 
  {
  ...
  task_YM1S1a <- sl3_Task$new(
    data = data.table::copy(df_YM1S1a),
    covariates = covariates$covariates_Y,
    outcome = "Y",
    outcome_type = "binomial"
  )
  
  task_YM0S1a <- sl3_Task$new(
    data = data.table::copy(df_YM0S1a),
    covariates = covariates$covariates_Y,
    outcome = "Y",
    outcome_type = "binomial"
  )
  
  task_ZS0a <- sl3_Task$new(
    data = data.table::copy(df_ZS0a),
    covariates = covariates$covariates_Z,
    outcome = "Z",
    outcome_type = "binomial"
  )
  ...
  Z_S0a_ps = Zfit$predict(task_ZS0a)
  Z_S0a_ps0 = with(df_ZS0a, truth$f_Z(A=A, W=W, S=S))
  ...
  M_S1a_ps = Mfit$predict(task_MS1a)
  ...
  Z_S1a_ps = Zfit$predict(task_ZS1a)
  Z_S1a_ps0 = with(df_YM1S1a, truth$f_Z(A=A, W=W, S=S))
  ...
  A_S1_ps = Afit$predict(task_AS1a)
  A_S1_ps0 = with(df_AS1a, truth$f_A(W=W, S=S))
  ...
  Hm1 = with(df_YM1S1a, (
    (S == 1)*(A == a)*
      ...
      ((Z == 1)*Z_S0a_ps + (Z == 0)*(1 - Z_S0a_ps))*
      S0_preds)/
      (((M == 1)*M_S1a_ps + (M == 0)*(1 - M_S1a_ps))*
         ((Z == 1)*Z_S1a_ps + (Z == 0)*(1 - Z_S1a_ps))*
         ((A == 1)*A_S1_ps + (A == 0)*(1 - A_S1_ps))*S1_preds*PS0))
  ...
  Y_preds_M1 = Yfit$predict(task_YM1S1a)
  Y_preds_M0 = Yfit$predict(task_YM0S1a)
  
  lrnr_glm = make_learner(Lrnr_glm_fast, family = binomial())
  Yfit_mle = lrnr_glm$train(task_YS1)
  Y_mle_preds = Yfit_mle$predict(task_Y)
  Y_mle_preds_M1 = Yfit_mle$predict(task_YM1S1a)
  Y_mle_preds_M0 = Yfit_mle$predict(task_YM0S1a)
