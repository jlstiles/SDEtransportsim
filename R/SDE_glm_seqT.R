# Simulation function
#' @export
SDE_glm_seqT = function(data, forms, RCT = 0.5,pooled, gstar_S = 0, truth, B = 500) 
{
  
  # get the stochastic dist of M and true params if you want 
  gstar_info = get_gstarM_glm_seqT(data = data, truth = truth, forms = forms, pooled, gstar_S)
  ...
  init_info = get.mediation.initdata_glm_seqT(data = data, forms = forms, RCT = RCT, pooled)
  Y_preds = init_info$Y_preds
  est_info = lapply(0:1, FUN = function(astar) {
    lapply(0:1, FUN = function(a) {
      # get tmle info
      # get iptw here while I'm at it
      update = mediation.step1_glm_seqT(initdata = init_info$initdata, init_info$Y_preds, data = data, 
      ...
      EE_gcomp_info = mediation.step2_glm_seqT(data = data, Qstar_M = Y_preds[[1]], 
      ...
      tmle_info = mediation.step2_glm_seqT(data = data, Qstar_M = update$Qstar_M, 
    ...
    boot_ests = lapply(1:B, FUN = function(x) {
      ...
      est_info = lapply(0:1, FUN = function(astar) {
        return(lapply(0:1, FUN = function(a) {
          update = mediation.step1_glm_seqT(initdata = init_info$initdata, Y_preds = Y_preds, data = data, 
                                       gstarM_astar[[astar+1]], a)
          iptw_info = update$est_iptw
          
          Y_Mg = get.stochasticM(gstarM_astar[[astar+1]], Y_preds[[2]], Y_preds[[3]]) 
          A_ps = init_info$initdata$A_ps
          EE_mle_info = mediation.step2_glm_seqT(data = data, Qstar_M = Y_preds[[1]], 
                                            Qstar_Mg = Y_Mg, Hm = update$Hm, A_ps = A_ps, 
                                            a = a, tmle = FALSE,
...
#' @export
get_gstarM_glm_seqT  = function(data, truth, forms, pooled, gstar_S) 
{
...
#' @export
get.mediation.initdata_glm_seqT = function(data, forms, RCT = 0.5, pooled) {
...
#' @export
mediation.step1_glm_seqT = function(initdata, Y_preds, data, gstarM_astar, a) {
