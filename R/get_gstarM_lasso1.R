...
get_gstarM_lasso1  = function(data, forms, Wnames, Wnamesalways, transport, 
                             pooled, 
                             gstar_S,
                             truth) 
{
  ...
  if (transport & !pooled) {
    Mstarfit = glmnet(dataMstar[data$S==gstar_S, ], data$M[data$S==gstar_S], family = "binomial")
    Zstarfit = glmnet(dataZstar[data$S==gstar_S, ], data$Z[data$S==gstar_S], family = "binomial")
  } else {
    Mstarfit = glmnet(dataMstar, data$M, family = "binomial")
    Zstarfit = glmnet(dataZstar, data$Z, family = "binomial")
  }
  ...
  if (!is.null(truth)) {
    ftruth = function(f_W, f_S, setA, f_Z, f_Y, transport, pooled) {
      ...
      pop_predZastar = predict(Zstarfit, newx = popZastar, type = 'response')
      
      popMz1 = model.matrix(Mstarform, popZ1)[,-1]
      pop_predMz1 = predict(Mstarfit, newx = popMz1, type = 'response')
      
      popMz0 = model.matrix(Mstarform, popZ0)[,-1]
      pop_predMz0 = predict(Mstarfit, newx = popMz0, type = 'response')
