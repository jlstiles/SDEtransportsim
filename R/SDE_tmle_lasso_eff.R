                          ...
                          pooled, gstar_S = 1, truth = NULL) 
{
  if (length(unique(data$Y)>2)) {
    ...
    init_info = get.mediation.initdata_lasso_eff(data = data, forms = forms, RCT = RCT, Wnames = Wnames, 
    ...
    est_info = lapply(0:1, FUN = function(astar) {
      lapply(0:1, FUN = function(a) {
        ...
        update = mediation.step1_lasso_eff(initdata = init_info$initdata, init_info$Y_preds, data = data, 
        ...
        tmle_info = mediation.step2_lasso_eff(data = data, Qstar_M = update$Qstar_M, 
