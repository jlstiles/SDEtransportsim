#' @export
mediation.steps_glm_eff = function(initdata, transport, bootstrap, max_iter) {
  tmledata = initdata
  calculateHQ = function(tmledata, setM, setZ, setS) {
    ...
    if (transport) {
      ...
      H_SDE = with(tmledata, (((setM == 1)*gstarM_astar0 + (setM == 0)*(1 - gstarM_astar0))
                              *((ZA1_ps*setZ+ (1 - setZ)*(1 - ZA1_ps))-(ZA0_ps*setZ+ (1 - setZ)*(1 - ZA0_ps)))/
                                (((setM == 1)*M_ps + (setM == 0)*(1 - M_ps))*
                                   (Z_Wps*setZ+ (1 - setZ)*(1 - Z_Wps)))))
    }
    
    if (transport) {
      H_SIE = with(tmledata, ((setS == 1)*
      ...
      H_SIE = with(tmledata, ((((setM == 1)*gstarM_astar1 + (setM == 0)*(1 - gstarM_astar1))-
                                 ((setM == 1)*gstarM_astar0 + (setM == 0)*(1 - gstarM_astar0)))   
                              *(ZA1_ps*setZ+ (1 - setZ)*(1 - ZA1_ps))/
                                (((setM == 1)*M_ps + (setM == 0)*(1 - M_ps))*
                                   (Z_Wps*setZ+ (1 - setZ)*(1 - Z_Wps)))))
    }
  ...
  for (i in 1:max_iter){
    its = i
    HQ = calculateHQ(tmledata, setM = NULL, setZ = NULL, setS = NULL)
    ...
    HM1Z1 = clfm(calculateHQ(tmledata, setM = 1, setZ = 1, setS = 1),dir)
    HM1Z0 = clfm(calculateHQ(tmledata, setM = 1, setZ = 0, setS = 1),dir)
    HM0Z1 = clfm(calculateHQ(tmledata, setM = 0, setZ = 1, setS = 1),dir)
    HM0Z0 = clfm(calculateHQ(tmledata, setM = 0, setZ = 0, setS = 1),dir)
    ...
    if (transport) {
      ...
      tmledata$Z_Wps = with(tmledata, ZA1S1_ps*AS1_ps + ZA0S1_ps*(1 - AS1_ps))
    } else {
