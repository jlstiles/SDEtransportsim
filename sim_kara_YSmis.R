...
f_A = function(S,W) {
  df = cbind(S = S, W)
  with(df, plogis(-2 * S - 0.7 * W1 - W2 + 1))
}
...
covariates = list(covariates_S = c("W1"),
...
res100_YSmis = mclapply(1:B, FUN = function(x) sim_kara(n, covariates, func_list), 
                       mc.cores = getOption("mc.cores", 24L))

save(res100_YSmis, func_list, covariates, file = "results/res100_YSmis.RData")

B = 1000
n=500

res500_YSmis = mclapply(1:B, FUN = function(x) sim_kara(n, covariates, func_list), 
                       mc.cores = getOption("mc.cores", 24L))

save(res500_YSmis, func_list, covariates, file = "results/res500_YSmis.RData")

B = 500
n=5000

res5000_YSmis = mclapply(1:B, FUN = function(x) sim_kara(n, covariates, func_list), 
                        mc.cores = getOption("mc.cores", 24L))

save(res5000_YSmis, func_list, covariates, file = "results/res5000_YSmis.RData")
...
save(res5000_YSmis, func_list, covariates, file = "results/res5000_YSmis1.RData")